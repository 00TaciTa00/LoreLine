...
import { era, withDb } from "@/lib/db";
import { pickColor } from "@/lib/colors";
import { serializeEra } from "@/lib/db/serialize";
...
// GET /api/worlds/:worldId/eras - 세계관의 상위 기간 목록
export async function GET(_request: NextRequest, { params }: RouteParams) {
  const { worldId } = await params;

  const eras = await withDb((db) =>
    db
      .select()
      .from(era)
      .where(and(eq(era.worldId, Number(worldId)), isNull(era.deletedAt)))
      .orderBy(asc(era.sortKey)),
  );

  return NextResponse.json({ eras: eras.map(serializeEra) });
}

// POST /api/worlds/:worldId/eras - 상위 기간 생성
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const created = await withDb(async (db) => {
    ...
    const [{ value: existingCount }] = await db
      .select({ value: count() })
      .from(era)
      .where(and(eq(era.worldId, worldIdNum), isNull(era.deletedAt)));

    // 새 기간은 목록 맨 뒤에 붙인다.
    const sortKey = await resolveSortKey(db, worldIdNum, eraOrder, {
      kind: "end",
    });
  ...
  return NextResponse.json({ era: serializeEra(created) }, { status: 201 });
}
