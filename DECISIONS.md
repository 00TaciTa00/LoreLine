...
> **이 결정은 2026-09-25에 대체되었다.** 아래 "작업 흐름: 이슈 → 브랜치 → PR"을 볼 것.
> 당시의 판단이므로 지우지 않고 남겨 둔다.
...
- **현재 상태**: `app/api` 아래 라우트 10개가 전부 무인증이다. 미들웨어도 없고
  세션·토큰을 보는 코드가 한 줄도 없다. 배포된 주소를 아는 사람은 누구나 모든
  세계관을 읽고 고치고 지울 수 있다.
- **판단**: 혼자 쓰는 사이트이고, 실제 서비스와 고도화에 앞서 구상하고 시험하는
  단계라 인증을 두지 않는다. **빠뜨린 것이 아니라 고른 것이다.** 이 문단이 없으면
  코드만 봐서는 둘을 구분할 수 없어서 남긴다.
- **다만 "닫혀 있다"는 뜻은 아니다.** Workers Builds가 `main`을 자동 배포하므로
  배포판은 인터넷에 떠 있고 저장소도 public이다. 지금 무사한 이유는 인증이 있어서가
  아니라 주소를 아무도 모르기 때문이다.
- **다시 볼 조건** — 하나라도 걸리면 인증이 다른 작업보다 먼저다.
  - 본인 말고 다른 사람에게 주소를 알려줄 때
  - 잃으면 곤란한 분량의 원고가 쌓였을 때
  - #15·#16처럼 배포·판매 형태를 검토할 때

### 작업 흐름: 이슈 → 브랜치 → PR

- 위의 "커밋만 하고 push는 하지 않음"을 대체한다. **이제 push한다.**
- 순서는 **이슈 → 브랜치 → 작업 → PR → CI 통과 → 머지 → 브랜치 삭제**다.
  문서 한 문단을 고치는 작은 일도 이슈부터 만든다. 브랜치 이름 규칙이
  이슈 번호를 요구하고(`00TaciTa00/<이슈번호>-<설명>`), 이슈 없이 시작하면
  무엇을 왜 했는지가 커밋 메시지에만 남아 추적이 끊긴다.
- PR 본문 끝에 `Closes #<번호>`를 두면 머지될 때 이슈가 함께 닫힌다.
- **`main`에 직접 push할 수 없다.** #29에서 브랜치 보호 규칙을 걸었다.
  - `check`(CI) 통과 필수. 서식 → 정적 검사 → 타입 → 테스트를 모두 돌린다
  - **관리자도 예외가 아니다**(`enforce_admins`). 예외를 두면 규칙이 없는 것과 같다
  - **리뷰 승인은 요구하지 않는다.** 혼자 쓰는 저장소라 자기 PR을 승인할 수 없어,
    걸어두면 머지가 영영 막힌다
  - 강제 푸시와 브랜치 삭제는 막아 두었다
  - "브랜치를 최신 상태로" 요구는 켜지 않았다. 순차로 작업하니 얻는 것이 적고,
    켜면 머지 때마다 CI가 다시 돌아 2분씩 더 든다
- Cloudflare Workers Builds도 PR마다 검사로 붙지만 **필수 체크는 `check` 하나뿐이다.**
  배포까지 머지 조건으로 삼지는 않는다.
